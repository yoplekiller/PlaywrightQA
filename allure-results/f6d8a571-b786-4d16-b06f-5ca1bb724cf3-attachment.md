...
- Name: test
- Location: C:\Users\jmlim\OneDrive\Desktop\PlaywrightQA\tests\ui\ui_search.spec.ts:3:5
...
Expected string: "[KF365] 햇 양파 1.8kg"
Received string: "컬리 - 마켓컬리/뷰티컬리"
Call log:
  - expect.toHaveTitle with timeout 5000ms
  - waiting for locator(':root')
    2 × locator resolved to <html lang="ko" data-theme="light">…</html>
      - unexpected value "검색결과 > 양파 - 마켓컬리"
    7 × locator resolved to <html lang="ko" data-theme="light">…</html>
      - unexpected value "컬리 - 마켓컬리/뷰티컬리"

    at C:\Users\jmlim\OneDrive\Desktop\PlaywrightQA\tests\ui\ui_search.spec.ts:11:22
```
...
- button:
  - img
- link "[KF365] 햇 양파 1.8kg":
  - /url: /goods/5049920
  - paragraph: "[KF365] 햇 양파 1.8kg"
- paragraph:
  - img
  - text: 냉장
- link:
...
- button:
  - img
- paragraph: 상품 5,490원 + 배송비 3,000원
- paragraph: 8,490원
- paragraph: 결제금액
- paragraph: 상품금액
- paragraph: 7,990원
...
```ts
   ...
   5 |   await page.locator('.css-vdi47h').click();
   6 |   await page.getByRole('textbox', { name: '검색어를 입력해주세요' }).fill('양파');
   7 |   await page.getByRole('textbox', { name: '검색어를 입력해주세요' }).press('Enter');
   8 |   await page.getByRole('link', { name: '담기 샛별배송 [KF365] 햇 양파 1.8kg' }).getByRole('button').click();
   9 |   await page.getByRole('button', { name: '장바구니 담기' }).click();
  10 |   await page.getByRole('button', { name: '1', exact: true }).click();
> 11 |   await expect(page).toHaveTitle("[KF365] 햇 양파 1.8kg")
     |                      ^ Error: Timed out 5000ms waiting for expect(locator).toHaveTitle(expected)
  12 | });
```