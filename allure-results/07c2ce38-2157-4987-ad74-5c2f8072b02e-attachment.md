...
Expected: "2"
Received: "샛별배송"
    at C:\Users\jmlim\OneDrive\Desktop\PlaywrightQA\tests\ui\ui_cart_duplicate.spec.ts:17:25
```
...
```ts
  ...
  14 |
  15 |  const quantity = await page.locator('p').nth(3).innerText();
  16 |  console.log(`수량: ${quantity}`);
> 17 |  await expect(quantity).toBe('2');
     |                         ^ Error: expect(received).toBe(expected) // Object.is equality
  18 | });
```