...
```
Error: locator.click: Error: strict mode violation: getByText('담기') resolved to 23 elements:
...
Call log:
  - waiting for getByText('담기')

    at C:\Users\jmlim\OneDrive\Desktop\PlaywrightQA\tests\ui\ui_cart_duplicate.spec.ts:11:30
```
...
```ts
  ...
  10 |
> 11 |   await page.getByText("담기").click();
     |                              ^ Error: locator.click: Error: strict mode violation: getByText('담기') resolved to 23 elements:
  12 |   await page.getByRole('button', { name: '장바구니 담기' }).click();
  13 |   await page.getByText("담기").click();
