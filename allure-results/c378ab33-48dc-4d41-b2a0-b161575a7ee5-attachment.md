...
Call log:
  - waiting for getByText('전체선택 0/')
    - locator resolved to <p class="kpds_97oqoup kpds_97oqouw kpds_97oqou6 kpds_ldmw177h kpds_97oqou1a kpds_97oqouk kpds_ldmw177x kpds_97oqou13 kpds_97oqoud kpds_ldmw177o kpds_126coma4 kpds_126coma6 kpds_126coma2 kpds_ldmw179c kpds_126coma8 kpds_126coma3 kpds_ldmw171a0">전체선택 0/0</p>
  - attempting click action
    ...
    - retrying click action
      - waiting 100ms
    265 × waiting for element to be visible, enabled and stable
        - element is not enabled
      - retrying click action
        - waiting 500ms

    at C:\Users\jmlim\OneDrive\Desktop\PlaywrightQA\tests\ui\ui_cart_product_cancel.spec.ts:40:35
```
...
- heading "장바구니" [level=2]:
  - paragraph: 장바구니
- checkbox "전체선택 0/0" [disabled]
- paragraph: 전체선택 0/0
- button "선택삭제" [disabled]:
  - paragraph: 선택삭제
- img
...
```ts
  ...
  22 |   await nextBtn.click();
  23 |
  24 |   const cartAddBtn = page.locator("xpath=(//button[@class='css-ahkst0 e4nu7ef3'])[1]");
  25 |   await expect(cartAddBtn).toBeVisible({ timeout: 7000 });
  26 |   await cartAddBtn.click();
  27 |
  28 |
  29 |   const cartBtn = page.locator("xpath=(//button[@class='css-g25h97 e14oy6dx1'])")
  30 |   await expect(cartBtn).toBeVisible({ timeout: 7000 });
  31 |   await cartBtn.click();
  32 |
  33 |   await expect(page).toHaveURL('https://www.kurly.com/cart');
  34 |
  35 |   // await page.getByRole('button', { name: '2', exact: true }).click();
  36 |   await page.getByText('전체선택 1/').click();
  37 |   await page.getByText('전체선택 0/').click();
  38 |   await page.getByRole('button', { name: '선택삭제' }).click();
  39 |   await page.getByRole('button', { name: 'confirm-button' }).click();
> 40 |   await page.getByText('전체선택 0/').click();
     |                                   ^ Error: locator.click: Test timeout of 150000ms exceeded.
  41 |   await expect(page.getByText('장바구니에 담긴 상품이 없습니다.')).toBeVisible();
  42 | });
```