...
<<<<<<< HEAD
test('이벤트트 팝업 기능', async ({ page }) => {
  allure.description('이벤트트 팝업이 정상적으로 표시되고, 쿠폰 버튼 클릭 시 쿠폰 페이지로 이동하는지 확인합니다.');
=======
test('이벤트 팝업 기능', async ({ page }) => {
  allure.description('이벤트 팝업이 정상적으로 표시되고, 쿠폰 버튼 클릭 시 쿠폰 페이지로 이동하는지 확인합니다.');
>>>>>>> develop
