...
test('🔐 로그인 후 메인 버튼 확인 테스트', async ({ page }, testInfo) => {
  ...
  // 로그인 입력
  await page.getByRole('textbox', { name: '아이디를 입력해주세요' }).fill('kurly_id'); // TODO: 실제 계정 정보로 교체
  await page.getByRole('textbox', { name: '비밀번호를 입력해주세요' }).fill('kurly_pw'); // TODO: 실제 계정 정보로 교체
